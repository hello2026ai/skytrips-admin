...
BEGIN
  ...
  -- 4. Fallback: Copy from "Travel Date" (CSV header style) if it exists
   ...
   -- 5. Fallback: Construct from Issue columns (IssueDay, issueMonth, issueYear)
   -- It appears data was imported into these columns but represents the travel date
   UPDATE public.bookings 
   SET "departureDate" = ("IssueDay" || ' ' || "issueMonth" || ' ' || "issueYear")
   WHERE "departureDate" IS NULL 
   AND "IssueDay" IS NOT NULL 
   AND "issueMonth" IS NOT NULL 
   AND "issueYear" IS NOT NULL;
